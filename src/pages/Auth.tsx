...
const Auth: React.FC = () => {
  ...
  const [authState, setAuthState] = useState<AuthState>(AuthState.LOGIN_REGISTER);
  ...
  const handleTermsCancelled = async () => {
    ...
    try {
      await logout();
      setAuthState(AuthState.LOGIN_REGISTER);
      setAuthError("You must accept the terms and conditions to use ICanCook.");
    } catch (error) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // After successful login, directly go to dashboard since they've already
      // accepted terms before
      setAuthState(AuthState.COMPLETE);
    } catch (error) {
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      // Check if this is first time login (for demo purposes, always show terms for Google)
      // In a real app, you'd check if they've accepted terms before
      proceedToTermsAgreement();
    } catch (error) {
  ...
  const handleAppleLogin = async () => {
    ...
    try {
      ...
      // Check if this is first time login (for demo purposes, always show terms for Apple)
      proceedToTermsAgreement();
    } catch (error) {
  ...
  // Show appropriate component based on current auth state
  ...
  if (authState === AuthState.TERMS_AGREEMENT) {
    return (
      <TermsAgreement 
        open={true} 
